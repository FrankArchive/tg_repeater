...
namespace tg_duxin {
    ...
    class OptimisticModuleManager { //主动式mod
        ...
        public static async void SendPhoto (ChatId chatId, string url) {
            try {
                await Program.repeater.SendPhotoAsync (chatId, url);
        ...
        public static async void SendVoice (ChatId chatId, string url) {
            try {
                await Program.repeater.SendAudioAsync (chatId, url);
            } catch (ApiRequestException) {
                await Program.repeater.SendTextMessageAsync (
                    chatId,
                    $"获取音乐失败，url为{url}"
    ...
    class PassiveModuleManager { //被动式mod
        ...
        public static void LoadModule () {
            ...
            pool.Add (new Module_hitokoto.Interface ());
        }
        public static void InitModule () {
            foreach (Module i in pool)
                i.submitCommands ();
            ...
            foreach (Module i in pool) {
                try {
                    if (i.required.IndexOf (message.Type) != -1 &&
                        ...
                        // 非文字消息如何处理？利用OptimisticModuleManager
                        try {
                            await Program.repeater.SendTextMessageAsync (
                                message.Chat.Id,
                                sendBack
                            ...
                            if (e is NotImplementedException) { } else if (e is FormatException) {
                                await Program.repeater.SendTextMessageAsync (
                                    message.Chat.Id,
                                    "命令格式错误"
                                );
