    ...
    public class CoolQHandle {
        public static void InitCoolQ () {
            if (Config.isStarted == false) {
                Config.lastSaid = new System.Collections.Generic.Dictionary<string, ReceivedMessage> ();
        ...
        public static async void SendTextMessage (Endpoint i, string message) => await Config.CoolQClient.SendMessageAsync (i, message);
        ...
        private async static void onMessageReceived (HttpApiClient api, Sisters.WudiLib.Posts.Message message) {
            ...
            if (message.Content.IsPlaintext) {
                try {
                    ...
                    if (result != "")
                        await api.SendMessageAsync (message.Endpoint, result);
                } catch { }

            }
            try {
                Config.lastSaid[message.Sender["nickname"].ToString ()] = message.Content;
            } catch { }
            try {
                ...
                switch (com.operation) {
                    ...
                    case 6: //getlast
                        if (com.parameters.Count != 1 || !Config.lastSaid.ContainsKey (com.parameters[0])) {
                            await api.SendMessageAsync (message.Endpoint, "爱莫能助");
                            return;
                        }
                        await api.SendMessageAsync (message.Endpoint, $"{com.parameters[0]} 刚才发了:");
                        await api.SendMessageAsync (message.Endpoint, Config.lastSaid[com.parameters[0]].Forward ());
                        return;
        ...
        public static string Ping() {
            using (var http = new HttpClient()) {
                var info = http.GetStringAsync(Config.apiAddr).Result;
                Config.CoolQClient.CleanImageData();
